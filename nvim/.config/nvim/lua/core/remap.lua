...
-- Define a Lua function to insert "start" and "end" around the visual selection
function insert_start_end_visual()
	...
	-- Insert "start" on a new line above the selection
	vim.fn.append(start_line - 1, "start_time = time.perf_counter()")

	-- Adjust the end_line since we added a line above
	end_line = end_line + 1

	-- Insert "end" on a new line below the selection
	vim.fn.append(end_line, "elapsed = time.perf_counter() - start_time")
	vim.fn.append(end_line + 1, "logger.info(f'Execution time: {elapsed:.2f} seconds')")
end

-- Create a Vim mapping to call the Lua function
vim.api.nvim_set_keymap('v', '<Leader>se', ':<C-u>lua insert_start_end_visual()<CR>', { noremap = true, silent = true })
